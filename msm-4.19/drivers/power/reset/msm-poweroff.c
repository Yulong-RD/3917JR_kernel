...
#ifdef CONFIG_YL_REBOOT_STRESS_TEST
	/* Set hard reset on reboot stress test via cmd "Received
	 * ACTION_REBOOT broadcast"
	 */
	if (cmd != NULL && !strncmp(cmd, "Received ACTION_REBOOT", 22)) {
...
#ifdef CONFIG_YL_DLOAD_MODE
	if (is_device_door_locked())
		download_mode = 0;
#endif
