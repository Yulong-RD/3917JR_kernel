...
static void device_restart_work_hdlr(struct work_struct *work)
{
...
#ifdef CONFIG_YL_RESERVED_RAM
	size_t size;
	char *smem_reason, reason[256];
	char *err_reason = "unknown, qcom_smem_get failed";
#endif
	...
	msleep(100);
#ifdef CONFIG_YL_RESERVED_RAM
	subsystem_failed = true;

	pr_emerg("subsys-restart: Resetting the SoC - %s crashed.",
							dev->desc->name);

	smem_reason = yl_get_failure_reason(dev->desc, &size);

	if (unlikely(!smem_reason)) {
		strlcpy(reason, err_reason, sizeof(reason));
		goto out_panic;
	}

	strlcpy(reason, smem_reason, size);
out_panic:
	panic("%s - %s", dev->desc->name, reason);
#else
