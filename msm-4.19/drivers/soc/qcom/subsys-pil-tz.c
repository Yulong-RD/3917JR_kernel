...
#if defined(CONFIG_YL_PSTORE_SSR_REASON) || defined(CONFIG_YL_RESERVED_RAM)
static char *get_smem_reason(const struct pil_tz_data *d, size_t *sz)
{
	size_t size;
	char *smem_reason;
	...
	*sz =  min(size, (size_t)MAX_SSR_REASON_LEN);

	return smem_reason;
}
#endif

#ifdef CONFIG_YL_RESERVED_RAM
char *yl_get_failure_reason(const struct subsys_desc *subsys, size_t *sz)
{
	struct pil_tz_data *d = subsys_to_data(subsys);

	return get_smem_reason(d, sz);
}
EXPORT_SYMBOL_GPL(yl_get_failure_reason);
#endif

#ifdef CONFIG_YL_PSTORE_SSR_REASON

#include <linux/yl_params.h>

void yl_store_failure_reason(const struct subsys_desc *subsys)
{
	size_t size;
	char *smem_reason, reason[MAX_SSR_REASON_LEN];
	struct pil_tz_data *d = subsys_to_data(subsys);

	smem_reason = get_smem_reason(d, &size);
	if (!smem_reason)
		return;

	strlcpy(reason, smem_reason, size);
	yl_pstore_ss_failure_reason(d->subsys_desc.name, reason);
}
EXPORT_SYMBOL_GPL(yl_store_failure_reason);
#endif
